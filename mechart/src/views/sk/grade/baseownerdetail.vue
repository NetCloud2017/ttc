    ...
    <div class='content-body'>
      <div class='content-body-one'>
        <div class='content-one-title'>
          ...
          <span>
            违规次数：
            <span style='color:#e63453; font-size: 14px;'>{{Data.result.totalViolateTime}}</span>
          </span>
        </div>
        <div class='content-one-main'>
          <div class='content-one-mi'>
            ...
            <div class='content-one-chi'>
              <span class='textt'>类别</span>
            </div>
            <div class='content-one-chi'>
              <span class='textt'>注册资本</span>
            </div>
            <div class='content-one-chi'>
              <span class='textt'>年营业额</span>
            </div>
            <div class='content-one-chi'>
              <span class='textt'>法人</span>
            </div>
            <div class='content-one-chi'>
              <span class='textt'>履约率</span>
            </div>
            <div class='content-one-chi'>
              <span class='textt'>历史交易金额</span>
            </div>
            <div class='content-one-chi'>
          ...
          <div class='content-one-mz'>
            <div class='content-one-chil'>
              <span class='textright' v-if='Data.result.ownerNo'>{{Data.result.ownerNo}}</span>
              <span class='textright' v-else>-</span>
            </div>
            <div class='content-one-chil'>
              <span class='textright' v-if='Data.result.ownerName'>{{Data.result.ownerName}}</span>
                <span class='textright' v-else>-</span>
            </div>
            <div class='content-one-chil'>
              <span class='textright' v-if="Data.result.ownerTypeName">{{Data.result.ownerTypeName}}</span>
                <span class='textright' v-else>-</span>
            </div>
            <div class='content-one-chil'>
              <span class='textright' v-if="Data.result.registCapital">{{Data.result.registCapital}}</span>
                <span class='textright' v-else>-</span>
            </div>
            <div class='content-one-chil'>
              <span class='textright' v-if='Data.result.annualTurnover'>{{Data.result.annualTurnover}}</span>
              <span class='textright' v-else>-</span>
            </div>
            <div class='content-one-chil'>
              <span class='textright' v-if='Data.result.legalPerson'>{{Data.result.legalPerson}}</span>
              <span class='textright' v-else>-</span>
            </div>
            <div class='content-one-chil'>
            ...
            <div class='content-one-chil'>
              <span class='textright' v-if="Data.result.historyTradeAmount">{{Data.result.historyTradeAmount}}</span>
              <span class='textright' v-else>-</span>
            </div>
            <div class='content-one-chil'>
      ...
      <div class='content-body-two'>
        <div class='content-two-title'>
          ...
          <span @click='returnowner' class='content-two-titler'>返回</span>
        </div>
        <div class='content-two-body'>
          <div class='content-two-text'>
            <p>{{Data.result.migReference}}</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class='bottom'>
      <div class='bottom-body-one'>
        <div class='page-project'>
          <div class='egrid-class' style='width: 100%; height: 390px'>
            <div
              class='project-title-bar'
              style=' width: 97%;height: 34px;margin-top: 0px; line-height: 34px;'
            >
              <span>历史交易记录</span>
            </div>
            <egrid
...
export default {
  ...
  data() {
    return {
      ...
      productSchema: {
        ...
        产品: { minWidth: 200, sortable: false, component: EgridCellView },
        类型: { minWidth: 100, sortable: false, component: EgridCellView },
        执行折扣率: { minWidth: 80, sortable: false, component: EgridCellView },
        仓储地: { minWidth: 120, sortable: false, component: EgridCellView },
        开始时间: { minWidth: 140, sortable: false, component: EgridCellView },
  ...
  created() {
    ...
    this.ajax({
      type: 'GET',
      url: `/malisk/client/owner/ownerDetail/${this.ownerId}`,
      data: {},
      loading: true,
      success: res => {
        setTimeout(() => {
